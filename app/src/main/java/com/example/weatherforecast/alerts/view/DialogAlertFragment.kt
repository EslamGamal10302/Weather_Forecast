    ...
    override fun onCreateView(
        ...
        binding.floatingDialogButton.setOnClickListener {
            if (selectedAlert.dateFrom == 0L || selectedAlert.dateTo == 0L || selectedAlert.timeFrom == 0L ||
                ...
                Toast.makeText(
                    requireContext(),
                    getString(R.string.alert_type_empty),
                    Toast.LENGTH_LONG
                )
                    .show()
            } else if (selectedAlert.dateFrom < (Calendar.getInstance().timeInMillis - 24 * 60 * 60 * 1000) || selectedAlert.dateTo < (Calendar.getInstance().timeInMillis - 24 * 60 * 60 * 1000)) {
                Toast.makeText(
                    requireContext(),
                    getString(R.string.passed_date),
                    Toast.LENGTH_LONG
                )
                    .show()
            } else {
                ...
                myListner = listner
                selectedAlert.id =
                    generateUniqueIntValue(selectedAlert.dateFrom, selectedAlert.dateTo, "", "")
        ...
        binding.dateFrom.setOnClickListener {
            DatePickerDialog(
                ...
                dateFromPicker,
                calendarDateFrom.get(Calendar.YEAR),
                calendarDateFrom.get(Calendar.MONTH),
                calendarDateFrom.get(Calendar.DAY_OF_MONTH)
            ).show()

        }
        binding.dateTo.setOnClickListener {
            DatePickerDialog(
                requireContext(), R.style.TimePickerTheme, dateToPicker,
                calendarDateTo.get(Calendar.YEAR), calendarDateTo.get(Calendar.MONTH),
                calendarDateTo.get(Calendar.DAY_OF_MONTH)
            ).show()
        }
        binding.hourFrom.setOnClickListener {
            TimePickerDialog(
                requireContext(), R.style.TimePickerTheme, timeStartPicker,
        ...
        binding.hourTo.setOnClickListener {
            TimePickerDialog(
                requireContext(), R.style.TimePickerTheme, timeEndPicker,
